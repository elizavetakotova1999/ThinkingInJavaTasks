...
import java.util.*;
// Управление и выполнение очереди задач./Managing and executing a queue of tasks.
...
public class TaskManager<R, C extends Callable<R>>
    ...
    public List<String> purge() {
        ...
        while (items.hasNext()) {
            TaskItem<R, C> item = items.next();
            // Оставляем выполненные задачи для отчета по результатам:
            if (!item.future.isDone()) {
                results.add("Cancelling " + item.task);
                item.future.cancel(true); // Может прерывать
                items.remove();
            }
        }
        return results;
